...
<%
	try{
		...
		if(result.equals("no")){
			%>	
		<script>alert("계정이 존재하지 않습니다. 다시 확인해주세요.")</script>
		<%
    ...
    <div class="login_containers">
        ...
        <form method="post" action="login.do" name = "frm">
            ...
            <div class="loginPassword">
                <input type="password" class="input" placeholder="비밀번호" name="pwd" maxlength="20">
            </div>
            <div class = "btn">
            <input type="button" class = "btn1" value="비밀번호 찾기" onclick ="location.href = 'pwdfind.jsp'"/>
            <input type="submit" class = "btn2" value="로그인" onclick = "return loginCheck()"/>
            </div>
        </form>
    </div>
